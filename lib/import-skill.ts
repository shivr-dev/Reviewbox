...
- pinyin: 系统不提供汉字输入框，使用显示答案后手动选择答对/不确定/答错的流程。prompt 给带声调拼音与必要消歧语境，不可提前泄露目标汉字；answer 为汉字。为有歧义的同音词给 acceptedAnswers 或充分语境。
...
- 每题必须有清楚的 explanation。题干、答案、解释与评分标准须一致。不要写 verified:true 或伪造核验结果。
